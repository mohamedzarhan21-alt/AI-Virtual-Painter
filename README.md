...
| Open palm (all five), held still 2 s | **Clear canvas** (undoable) |
| Thumb + index pinch, out in front | **Undo** |
| Thumb + middle pinch, out in front | **Redo** |
| Victory held still 2 s | **Save artwork** |

Two of these are deliberately harder to trigger than the rest, because both destroy
work:

- **Clear** needs a genuinely open hand — thumb included — held *still*, and the hand
  must have been doing something else first. An open hand simply resting in frame will
  never wipe the canvas, and holding the palm open clears once, not repeatedly.
- **Pinches** must happen out in front of the hand, where the two fingertips actually
  meet. A drawing hand rests with its spare fingers curled and the thumb lying against
  them; that is close, but tucked against the palm, and is not read as a pinch.
...
| `AIVP_FULLSCREEN` | `1` | Fill the screen at its native resolution |
| `AIVP_CANVAS_WIDTH` / `_HEIGHT` | screen size | Force a specific canvas resolution |
| `AIVP_SMOOTHING` | `0.2` | `0` = most responsive, `1` = smoothest |
| `AIVP_INFERENCE_WIDTH` | `640` | Width the frame is downscaled to for tracking |
| `AIVP_EXPOSURE` | automatic | Manual exposure, e.g. `-6`, for a faster sharper feed |

Invalid values are logged and ignored rather than crashing the application.

### Tuning responsiveness

`AIVP_SMOOTHING` is the single knob for how the fingertip feels. It maps onto the
1-Euro filter's cutoff and speed coefficient:

```bash
set AIVP_SMOOTHING=0.0    # snaps to the fingertip; a little jitter when you hover
set AIVP_SMOOTHING=0.5    # calmer lines, slightly more lag
```

For a smaller, faster window rather than fullscreen:

```bash
set AIVP_FULLSCREEN=0
```
...
## If tracking feels poor

**Lighting is almost always the answer, and it is not a software problem.** In a dim
room the webcam lengthens its exposure. That does two things at once: it caps the frame
rate — this camera drops from ~20 fps to 10 — and it smears the hand with motion blur.
Fewer, blurrier frames is the worst possible input for a hand tracker, and it is what
makes strokes break up and gestures misfire.

The interface shows this directly. The `CAM` line in the top-left telemetry is the rate
the camera is actually delivering; below 15 fps it turns amber and a `LOW LIGHT` warning
appears. If you see it:

1. **Add light on your hand** — a lamp facing you, not behind you. This is worth more
   than every setting below combined.
2. Avoid backlighting. A bright window behind you forces the camera to darken everything
   in front of it, including your hand.
3. If your camera supports manual exposure, `AIVP_EXPOSURE=-6` forces a short shutter.
   The picture gets darker but the feed gets faster and sharper, which the tracker
   prefers. Not every webcam honours it.

The application is built to cope with imperfect tracking — strokes survive dropped
frames, poses have hysteresis, destructive gestures need deliberate intent — but it
cannot invent hand positions that the camera never captured.
...
adapts its cutoff to the signal's own speed and gets both. `AIVP_SMOOTHING` moves both of
its coefficients together so the feel is one knob rather than two.

**Landmarks live in canvas space, never camera space.** MediaPipe returns normalised
coordinates, and the camera image is stretched to fill the canvas, so the canvas is the
only coordinate system the rest of the application uses. Converting with the *camera*
size instead put every landmark at two-thirds of its true position once fullscreen made
the two resolutions differ — far enough up and to the left that the wireframe sat on the
user's head while their hand went untracked. It looked exactly like the tracker latching
onto a face, and it was not: the tracker was right and the arithmetic was wrong. The
regression test pins a 720p camera against a 1080p canvas.

**Confidence thresholds: strict detection, default tracking.** Detection is deliberately
strict so nothing is adopted unless it is convincingly a hand. Presence and tracking are
left at their defaults rather than loosened, so a track that stops looking like a hand is
dropped promptly and detection can re-run. Holding on through the resulting gaps is this
application's job — `StrokeDriver` and the stale-observation window — not the
landmarker's. A geometric plausibility check backs this up: fingertips that do not sit a
hand-like distance from the wrist never reach the gesture layer.

**A stroke is not ended the moment tracking wavers.** `StrokeDriver` holds an open stroke
through a short grace period and rejoins it if the fingertip reappears nearby — the
difference between one continuous line and a row of fragments. Reappearing far away still
starts a new mark, so a trip to the palette never draws a bridge across the canvas.

**Destructive gestures need intent, not just a pose.** Clearing requires an armed hand
(one seen doing something else first), a settled lock-on, all five fingers, and two
seconds of stillness. Pinches must happen out in front of the hand where the fingertips
genuinely meet. Both rules came from watching the app misfire against a live camera: an
open hand merely resting in frame was wiping the canvas, and a drawing hand's thumb lying
against its curled middle finger fired redo seven times in twenty seconds.
...
| Stage | 1280×720 windowed | 1920×1080 fullscreen |
|---|---|---|
| Interface render | 9.7 ms | 13.5 ms |
| Tk display upload | 9.9 ms | 22.6 ms |
| Backdrop + artwork composite | 2.3 ms | 3.9 ms |
| **Full loop** | **22.9 ms** | **~46 ms** |
| **Measured frame rate** | **~40–44 FPS** | **~18–21 FPS** |
| Hand inference *(background thread)* | ~20 ms | ~25 ms |

Fullscreen costs roughly half the frame rate, and the reason is Tk: its photo upload
scales with the window's pixel count and is ~22 ms at 1080p, which nothing in Python can
avoid. The canvas is therefore rendered at the display's *native* resolution — letterboxing
a smaller canvas would pay the same upload cost **plus** a rescale, so native is strictly
faster as well as sharper. `AIVP_FULLSCREEN=0` returns to the faster windowed mode.

In practice this matters less than it looks: the camera here delivers 10–20 fps, so the
renderer is still running ahead of the only source of new hand data.
...
- **Tracking quality is bounded by the camera.** In poor light the feed drops to ~10 fps
  with heavy motion blur, and no amount of filtering recovers hand positions that were
  never captured. See [If tracking feels poor](#if-tracking-feels-poor).
- **Fullscreen halves the frame rate** because of Tk's display upload at 1080p.
