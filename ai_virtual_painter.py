...
#: Canvas size used when the window is not filling the screen.
DEFAULT_CANVAS: Final[tuple[int, int]] = (1280, 720)
...
def _env_float(name: str, default: float, low: float, high: float) -> float:
    """Read a float from the environment, constrained to ``[low, high]``."""
    raw = os.environ.get(name)
    if raw is None:
        return default
    try:
...
class AppConfig:
    ...
    #: Canvas size. ``0`` means "match the screen", resolved once the window exists.
    canvas_width: int = 0
    canvas_height: int = 0
    #: Width the frame is downscaled to before hand inference. Higher is more
    #: accurate; inference runs off the UI thread, so this buys tracking stability
    #: rather than costing frame rate.
    inference_width: int = 640
    target_fps: int = 60
    #: Start filling the screen at its native resolution.
    fullscreen: bool = True
    #: Pointer smoothing, 0 = most responsive, 1 = smoothest. Low values track the
    #: fingertip tightly at the cost of a little jitter when the hand is still.
    smoothing: float = 0.2
    #: Manual exposure value. ``None`` leaves the camera on automatic; a negative
    #: value such as -6 forces a short shutter for a faster, sharper feed.
    ...
    def from_environment(cls) -> AppConfig:
        ...
        return cls(
            ...
            canvas_width=_env_int("AIVP_CANVAS_WIDTH", 0, minimum=0),
            canvas_height=_env_int("AIVP_CANVAS_HEIGHT", 0, minimum=0),
            inference_width=_env_int("AIVP_INFERENCE_WIDTH", 640),
            fullscreen=_env_flag("AIVP_FULLSCREEN", True),
            smoothing=_env_float("AIVP_SMOOTHING", 0.2, 0.0, 1.0),
            exposure=_env_float("AIVP_EXPOSURE", 0.0, -16.0, 16.0) if "AIVP_EXPOSURE" in os.environ else None,
...
class StrokeDriver:
    """Owns the lifecycle of the stroke being drawn.

    Hand tracking is not continuous: a frame is occasionally missed, and a finger
    hovering near the edge of "extended" can flip the pose for a moment. Ending a
    stroke the instant either happens is what turns a smooth line into a row of
    fragments, so a stroke is instead held open briefly and rejoined if the
    fingertip reappears near where it left off.
    """

    #: How long an open stroke survives a lost pointer or a wavering pose.
    GRACE: Final = 0.28
    #: The fingertip must reappear within this radius to rejoin the same stroke;
    #: further away it is a new mark, not a resumed one.
    RESUME_RADIUS: Final = 190.0
    #: Minimum fingertip travel before a point is recorded. Small, so slow
    #: careful curves and fine detail are captured faithfully.
    MIN_POINT_DISTANCE: Final = 0.8

    def __init__(self, document: PaintDocument) -> None:
        self._document = document
        self._tool = Tool.IDLE
        self._last_point: tuple[float, float] | None = None
        self._grace_until = 0.0

    @property
    def holding(self) -> bool:
        """Whether a stroke is open but currently waiting out a dropout."""
        return self._grace_until > 0.0 and self._document.is_drawing
    ...
    def update(
        ...
        """Advance the stroke by one frame.

        Returns the name of the recognised shape when a stroke just ended and was
        snapped, otherwise ``None``.
        """
        if tool not in (Tool.DRAW, Tool.ERASE) or pointer is None:
            return self._coast(tool, now, snap_shapes)

        if not self._continues(tool, pointer):
            if self._document.is_drawing:
                self._document.end_stroke(snap_shapes=snap_shapes)
            self._document.begin_stroke(settings, eraser=tool is Tool.ERASE)
            self._last_point = None
        self._grace_until = 0.0
        self._tool = tool
        if self._last_point is None or math.dist(pointer, self._last_point) >= self.MIN_POINT_DISTANCE:
            self._document.extend_stroke(pointer[0], pointer[1], velocity)
            self._last_point = pointer
        return None

    def cancel(self) -> None:
        """Drop the open stroke and forget the grace state."""
        self._document.cancel_stroke()
        self._last_point = None
        self._grace_until = 0.0

    def _continues(self, tool: Tool, pointer: tuple[float, float]) -> bool:
        return (
            self._document.is_drawing
            and self._tool is tool
            and self._last_point is not None
            and math.dist(pointer, self._last_point) <= self.RESUME_RADIUS
        )

    def _coast(self, tool: Tool, now: float, snap_shapes: bool) -> str | None:
        """Hold an open stroke through a dropout, or finally close it."""
        if not self._document.is_drawing:
            self._tool = tool
            self._grace_until = 0.0
            return None
        if self._grace_until == 0.0:
            self._grace_until = now + self.GRACE
            return None
        if now < self._grace_until:
            return None
        self._tool = tool
        shape = self._document.end_stroke(snap_shapes=snap_shapes)
        self._last_point = None
        self._grace_until = 0.0
        return shape
...
class CameraStream:
    ...
    def __init__(self, config: AppConfig) -> None:
        ...
        self._frames_read = 0
        self._arrivals: deque[float] = deque(maxlen=40)
    ...
    def measured_fps(self) -> float:
        """Frame rate the device is actually delivering.

        Worth watching: webcams lengthen their exposure in dim light, which both
        caps this rate and adds the motion blur that makes a hand hard to track.
        A low number here is a lighting problem, not a software one.
        """
        with self._lock:
    ...
    def start(self) -> None:
        ...
        if self._config.exposure is not None:
            # Manual exposure trades image brightness for a shorter shutter, which
            # raises the frame rate and removes the motion blur that defeats hand
            # tracking. Not every device honours it, hence the readback.
            capture.set(cv2.CAP_PROP_AUTO_EXPOSURE, 0.25)
            capture.set(cv2.CAP_PROP_EXPOSURE, self._config.exposure)
            LOGGER.info(
                "Requested manual exposure %.1f (device reports %.1f)",
    ...
    def _run(self) -> None:
        ...
        while not self._stop.is_set():
            ...
            if not self._warned_slow and self._frames_read == 40:
                rate = self.measured_fps
                if rate < 15.0:
                    self._warned_slow = True
                    LOGGER.warning(
                        "Camera is delivering only %.1f fps. This is almost always dim lighting: "
                        "the sensor lengthens its exposure, which caps the frame rate and blurs "
                        "motion, making the hand harder to track. Brighter, more even light on "
                        "your hand is the single biggest improvement available. "
                        "AIVP_EXPOSURE can force a shorter exposure if your camera supports it.",
                        rate,
                    )
...
class HandTracker:
    ...
    def __init__(self, config: AppConfig, canvas_size: tuple[int, int]) -> None:
        self._config = config
        # Landmarks are reported in canvas pixels, not camera pixels. MediaPipe
        # returns normalised coordinates, and the camera image is stretched to fill
        # the canvas, so the canvas is the only space the rest of the application
        # ever works in. Scaling by the camera size instead silently offsets every
        # landmark whenever the two resolutions differ.
    ...
    def consecutive_misses(self) -> int:
        """Frames since the last successful detection."""
        return self._misses
    ...
    def start(self) -> None:
        ...
        options = mp_vision.HandLandmarkerOptions(
            ...
            num_hands=1,
            # Detection stays strict so nothing new is adopted unless it is
            # convincingly a hand. Presence and tracking are left at their defaults
            # rather than loosened: a track that stops looking like a hand should be
            # dropped promptly so detection can re-run. Continuity across the
            # resulting gaps is this application's job, not the landmarker's — see
            # StrokeDriver and the stale-observation window.
    ...
    def _run(self) -> None:
        ...
        while not self._stop.is_set():
            ...
            height, width = frame.shape[:2]
            scale = scale_target / float(width)  # inference resolution only
            ...
            observation = self._to_observation(result, *self._canvas_size)
            if observation is None:
                # A miss is usually a single bad frame - motion blur, a finger
                # crossing the palm - not the hand leaving. Publishing None here
                # would tear the stroke in two, so the previous observation is
                # left in place and allowed to age out through its timestamp.
                self._misses += 1
                continue
            self._misses = 0
            with self._lock:
    ...
    #: A real hand's fingertips sit between these multiples of the palm width away
    #: from the wrist, in any pose. A detection outside that range is not a hand.
    PLAUSIBLE_TIP_SPAN: Final[tuple[float, float]] = (0.45, 3.6)
    #: Below this the landmarks are too small to measure reliably.
    MIN_PALM_PIXELS: Final[float] = 12.0

    @classmethod
    def is_plausible_hand(cls, landmarks: np.ndarray, palm: float) -> bool:
        """Reject detections whose geometry could not belong to a hand.

        The landmarker occasionally locks onto a face or a busy background, and
        such a detection reads as an open palm — which, held for two seconds,
        wipes the canvas. Checking the shape costs nothing and rules that out.
        """
        if palm < cls.MIN_PALM_PIXELS:
            return False
        spans = np.linalg.norm(landmarks[list(FINGER_TIPS)] - landmarks[WRIST], axis=1) / palm
        low, high = cls.PLAUSIBLE_TIP_SPAN
        return bool(spans.min() >= low and spans.max() <= high)

    @classmethod
    def _to_observation(
        cls, result: mp_vision.HandLandmarkerResult, canvas_width: int, canvas_height: int
    ) -> HandObservation | None:
        """Convert a result into canvas-space landmarks, or ``None`` if unusable."""
        hands = getattr(result, "hand_landmarks", None)
        if not hands:
            return None
        landmarks = np.asarray(
            [(point.x * canvas_width, point.y * canvas_height) for point in hands[0]], dtype=np.float32
        )
        palm = float(np.linalg.norm(landmarks[MIDDLE_MCP] - landmarks[WRIST]))
        if not cls.is_plausible_hand(landmarks, palm):
            return None
...
class GestureEngine:
    ...
    #: A pinch must survive this long before it counts. A blurred or partly hidden
    #: hand can momentarily collapse two fingertips together, and an undo fired
    #: from that is indistinguishable from the app losing the user's work.
    PINCH_HOLD: Final[float] = 0.10
    #: A real pinch happens out in front of the hand: both tips meet at least this
    #: far from the wrist, in palm widths. A drawing hand rests with its spare
    #: fingers curled and the thumb lying against them — close tips, but tucked
    #: against the palm — and that must not read as a pinch.
    PINCH_MIN_REACH: Final[float] = 1.35
    ...
    STALE_SECONDS: Final[float] = 0.55
    #: Finger extension uses hysteresis: a finger must pass the upper ratio to
    #: count as raised and fall below the lower one to count as folded. A single
    #: threshold makes fingers chatter near the boundary, which is what turns a
    #: drawing hand into an erasing one without the user moving.
    EXTEND_UP: Final = (1.16, 1.09, 1.09, 1.09, 1.06)
    EXTEND_DOWN: Final = (1.06, 1.00, 1.00, 1.00, 0.98)
    #: How long each pose must persist before it takes effect. Drawing engages
    #: almost instantly; the destructive poses are made deliberately reluctant.
    POSE_DEBOUNCE: Final[dict[HandPose, float]] = {
        HandPose.DRAW: 0.05,
        HandPose.SELECT: 0.13,
        HandPose.ERASE: 0.26,
        HandPose.PALM: 0.30,
        HandPose.UNKNOWN: 0.16,
        HandPose.NONE: 0.16,
    }
    DEFAULT_DEBOUNCE: Final[float] = 0.14
    #: A newly acquired hand must be tracked this long before it is trusted with
    #: the destructive clear gesture.
    LOCK_ON_SECONDS: Final[float] = 0.9

    def __init__(self, smoothing: float = 0.2) -> None:
        # Map 0..1 onto the 1-Euro filter's two knobs. A low cutoff is glassy but
        # laggy; a high one tracks the fingertip almost immediately.
        level = clamp(smoothing, 0.0, 1.0)
        self._pointer_filter = PointFilter(
            min_cutoff=lerp(3.4, 0.9, level),
            beta=lerp(0.040, 0.008, level),
        )
        self._fingers: tuple[bool, bool, bool, bool, bool] = (False,) * 5
        self._skeleton: np.ndarray | None = None
        self._previous_pointer: tuple[float, float] | None = None
        self._previous_time = 0.0
        self._velocity = 0.0
        self._pinch_since: dict[str, float] = {}
        self._pinch_fired: dict[str, bool] = {}
    ...
    def reset(self) -> None:
        ...
        self._palm_armed = False
        self._pinch_since.clear()
        self._pinch_fired.clear()
        self._trail.clear()
    ...
    def update(self, observation: HandObservation | None, now: float) -> GestureState:
        ...
        # An open hand physically cannot be pinching, so undo and redo are simply
        # not on offer in that pose. This removes a whole class of false trigger.
        if stable is not HandPose.PALM:
            events.extend(self._detect_pinches(landmarks, palm, pinch_index, pinch_middle, now))
    ...
    def _extended_fingers(self, landmarks: np.ndarray) -> tuple[bool, bool, bool, bool, bool]:
        """Radial extension test with hysteresis — rotation invariant."""
        ...
        for index, ratio in enumerate(ratios):
            was_up = self._fingers[index]
            if ratio > self.EXTEND_UP[index]:
                state.append(True)
            elif ratio < self.EXTEND_DOWN[index]:
                state.append(False)
            else:
                state.append(was_up)  # inside the dead band, hold the last decision
        self._fingers = tuple(state)  # type: ignore[assignment]
    ...
    def _detect_pinches(
        self, landmarks: np.ndarray, palm: float, index_ratio: float, middle_ratio: float, now: float
    ) -> list[GestureEvent]:
        """Fire undo/redo once per sustained, deliberate pinch."""
        events: list[GestureEvent] = []
        ...
        candidates = (
            ("index", index_ratio, index_closer, INDEX_TIP, GestureEvent.UNDO),
            ("middle", middle_ratio, not index_closer, MIDDLE_TIP, GestureEvent.REDO),
        )
        wrist = landmarks[WRIST]
        for name, ratio, preferred, tip_index, event in candidates:
            meeting_point = (landmarks[THUMB_TIP] + landmarks[tip_index]) / 2.0
            reach = float(np.linalg.norm(meeting_point - wrist)) / palm
            if ratio < self.PINCH_ON and preferred and reach >= self.PINCH_MIN_REACH:
                since = self._pinch_since.get(name)
                if since is None:
                    self._pinch_since[name] = now
                elif not self._pinch_fired.get(name, False) and now - since >= self.PINCH_HOLD and cooled:
                    self._pinch_fired[name] = True
                    self._last_pinch_time = now
                    events.append(event)
            elif ratio > self.PINCH_OFF:
                self._pinch_since.pop(name, None)
                self._pinch_fired.pop(name, None)
        return events

    @staticmethod
    def _classify_pose(fingers: tuple[bool, bool, bool, bool, bool]) -> HandPose:
        thumb, index, middle, ring, pinky = fingers
        # The clear gesture demands a genuinely open palm, thumb included. Four
        # fingers alone are too easy to hit by accident, and the cost of a false
        # positive here is the whole canvas.
    ...
    def _debounce(self, pose: HandPose, now: float) -> HandPose:
        ...
        elif now - self._pose_since >= self.POSE_DEBOUNCE.get(pose, self.DEFAULT_DEBOUNCE):
    ...
    def _update_hold(self, pose: HandPose, stationary: bool, now: float) -> tuple[str, float, GestureEvent | None]:
        ...
        if pose not in (HandPose.PALM, HandPose.NONE):
            # Seeing the hand in any other pose arms the clear gesture. Opening the
            # palm then becomes a deliberate act rather than a state the hand
            # happens to be resting in.
            self._palm_armed = True
        settled = self._tracking_since is not None and now - self._tracking_since >= self.LOCK_ON_SECONDS
        if pose is HandPose.PALM and settled and stationary and self._palm_armed:
            # Clearing demands a hand that was doing something else a moment ago,
            # has been tracked long enough to trust, and is now deliberately still.
            # An open hand merely resting in frame is not a request to erase.
        ...
        if progress >= 1.0 and not self._hold_fired:
            ...
            if target is HandPose.PALM:
                self._palm_armed = False  # a second clear needs a fresh, deliberate gesture
                return label, 1.0, GestureEvent.CLEAR
            return label, 1.0, GestureEvent.SAVE
        if self._hold_fired:
            return "", 0.0, None
...
class HudRenderer:
    ...
    def _draw_telemetry(self, frame: np.ndarray, model: HudModel, alpha: float) -> None:
        # The camera rate is shown because it, not the renderer, is what usually
        # limits how well the app draws: a dim room means a long exposure, which
        # means few blurry frames for the tracker to work with.
        slow_camera = 0.0 < model.camera_fps < 15.0
        lines = (
            (f"FPS {model.fps:5.1f}", THEME.primary),
            (f"CAM {model.camera_fps:5.1f}", THEME.amber if slow_camera else THEME.text_mid),
            (f"HAND {model.inference_ms:5.1f}MS", THEME.text_mid),
            (f"STROKES {model.stroke_count:04d}", THEME.text_mid),
            (f"UNDO {'READY' if model.can_undo else '-----'}", THEME.text_mid),
            (f"REDO {'READY' if model.can_redo else '-----'}", THEME.text_mid),
        )
        ...
        if slow_camera:
            TEXT.draw(
                frame,
                "LOW LIGHT - ADD LIGHT FOR BETTER TRACKING",
                (self.TELEMETRY_X, 74 + len(lines) * 15 + 4),
                role=FontRole.MONO,
                size=9,
                color=THEME.amber,
                tracking=1,
...
class VirtualPainterApp:
    ...
    def __init__(self, config: AppConfig) -> None:
        ...
        self._width: int = 0
        self._height: int = 0
        # The window is created first so the canvas can be sized to the screen's
        # true pixel grid. Rendering at the display's native resolution keeps the
        # artwork sharp and skips a per-frame rescale on the way to the screen.
        self._create_window()
        self._document = PaintDocument(self._width, self._height)
        self._strokes = StrokeDriver(self._document)
        self._exporter = ArtworkExporter(config)
        self._hud = HudRenderer(self._width, self._height)
        self._gestures = GestureEngine(smoothing=config.smoothing)
        self._camera = CameraStream(config)
        self._tracker = HandTracker(config, (self._width, self._height))
    ...
    def _create_window(self) -> None:
        """Create the window and settle on the canvas resolution."""
        ...
        if self._config.canvas_width and self._config.canvas_height:
            self._width, self._height = self._config.canvas_width, self._config.canvas_height
        elif self._config.fullscreen:
            self._width, self._height = screen_w, screen_h
        else:
            self._width, self._height = DEFAULT_CANVAS
        LOGGER.info("Canvas %dx%d on a %dx%d screen", self._width, self._height, screen_w, screen_h)
    ...
    def _screen_size(self) -> tuple[int, int]:
        """Return the screen size in physical pixels.

        Tk reports screen metrics in DPI-scaled units, so on a 125 % display a
        1920x1080 panel comes back as 1536x864. Multiplying by the same factor
        CustomTkinter applies to geometry recovers the real pixel grid.
        """
        try:
        ...
        if scaling <= 0.01:
            scaling = 1.0
        return (
            max(640, round(self._root.winfo_screenwidth() * scaling)),
            max(480, round(self._root.winfo_screenheight() * scaling)),
        )
    ...
    def _tick(self) -> None:
        ...
        if boot >= 1.0:
            ...
            self._update_painting(gesture, started)
            self._update_dwell(gesture, started)
        else:
            self._strokes.cancel()
    ...
    def _build_model(self, gesture: GestureState, now: float, boot: float) -> HudModel:
        ...
        return HudModel(
            ...
            tool=gesture.tool,
            fps=self._fps,
            camera_fps=self._camera.measured_fps,
    ...
    def _update_painting(self, gesture: GestureState, now: float) -> None:
        """Advance the stroke driver and report any snapped shape."""
        shape = self._strokes.update(
            gesture.tool,
            gesture.pointer,
            gesture.velocity,
            self._settings,
    ...
    def _enter_fullscreen(self) -> None:
        """Fill the screen. The canvas is unchanged; only the window grows."""
        if self._fullscreen:
            return
        self._windowed_geometry = self._root.geometry()
        request = self._physical_size(*self._screen_size())
        self._root.geometry(f"{request[0]}x{request[1]}+0+0")
        self._fullscreen = True

    def _toggle_fullscreen(self) -> None:
        if self._fullscreen:
            fallback = self._physical_size(*DEFAULT_CANVAS)
            self._root.geometry(self._windowed_geometry or f"{fallback[0]}x{fallback[1]}+80+60")
            self._fullscreen = False
        else:
            self._enter_fullscreen()
